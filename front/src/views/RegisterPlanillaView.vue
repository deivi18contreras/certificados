...
const handleSubmit = async () => {
  ...
  try {
    ...
    const entidadFinal = entidadNormalizada === 'aportesenlinea' ? 'aportesEnLinea' : entidadNormalizada;

    const resReporte = await registerReporte({
      numPlanilla: form.value.numPlanilla,
      mesPagado: form.value.mesPagado.toLowerCase(),
      ...
      <q-form @submit="handleSubmit" class="q-pa-lg">
        ...
        <div class="row q-col-gutter-md">
          ...
          <div class="col-12 col-sm-8">
            <q-input v-model.number="form.numeroDoc" type="number" label="Número de Documento" outlined dense required />
          </div>
          <div class="col-12 col-sm-6">
            <q-select v-model="form.eps" :options="['Sanitas', 'Sura', 'Compensar', 'Salud Total', 'Nueva EPS']" label="EPS" outlined dense required />
          </div>
          <div class="col-12 col-sm-6">
            <q-input v-model="form.expCedula" type="date" label="Fecha Expedición Cédula" stack-label outlined dense />
          </div>
        </div>
        ...
        <div class="row q-col-gutter-md">
          <div class="col-12 col-sm-6">
            <q-input v-model.number="form.numPlanilla" type="number" label="Número de Planilla" outlined dense required />
          </div>
          <div class="col-12 col-sm-6">
            <q-select v-model="form.mesPagado" :options="months" label="Mes a Reportar" outlined dense required />
          </div>
          <div class="col-12 col-sm-6">
            <q-input v-model.number="form.valorPagado" type="number" label="Valor Pagado" outlined dense prefix="$" />
          </div>
          <div class="col-12 col-sm-6">
